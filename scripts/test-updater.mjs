...
try {
  ...
  sourceServer = createServer((_request, response) => {
    ...
    response.end(`<html><title>更新测试</title><body>点赞：11。${conceptName} 是什么？它在项目中如何使用？</body></html>`);
  });
  ...
  const sourceUrl = `http://127.0.0.1:${address.port}/interview`;
  ...
  const first = await updater.runAnalysis({ autoFetch: false, manualUrls: [sourceUrl, sourceUrl], aiChat, budgetMs: 60_000 });
  assert.equal(first.draft.newConcepts.length, 1, "first analysis should create one concept");
  ...
  const appliedFirst = await updater.applyUpdate({ selectedSourceIds: [sourceId], selectedConceptNames: [conceptName], lastRun: first });
  assert.equal(appliedFirst.counts.total, baselineCount + 5, "one concept should add five questions");
  ...
  const second = await updater.runAnalysis({ autoFetch: false, manualUrls: [sourceUrl], aiChat, budgetMs: 60_000 });
  ...
  assert.deepEqual(await snapshotBusinessFiles(), afterFirstApply, "repeat preview must remain read-only");
  ...
  assert.deepEqual(await snapshotBusinessFiles(), afterFirstApply, "rollback must byte-restore the state before the latest apply");
  ...
  console.log("Updater regression passed: read-only preview, new concept, repeat-run dedup, evidence-only apply, byte-equivalent rollback, unique IDs, cancellation.");
} finally {
